...
public class VoxelRenderer extends Module{
	...
	public VoxelRenderer() {
		...
		/*
		 * if (cave.getValue(wx, y, wz, 0.003f) / 1.1f + cave2.getValue(wx, y,
		 * wz, 0.006f) / 2.2f + Noise.normalNoise(wx, y, wz, 60f, 0.6f) +
		 * Noise.normalNoise(wx, y, wz, 25f, 0.2f) + Noise.normalNoise(wx, y,
		 * wz, 11f, 0.2f) >= 1.3f) voxels[wx][y][wz] = 0;
		 */
		...
		for(int wx = 0; wx < size; wx++){
			for(int wz = 0; wz < size; wz++){
				double theight = 150 + Noise.snoise(wx, wz, 70, 50) + Noise.snoise(wx, wz, 13, 7) + Noise.snoise(wx, wz, 5, 6) + ridges.getValue(wx, wz, 0.003f) * 45;
